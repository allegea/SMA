 ...
 *
 */
public class IndicadorPromediosSimplesGrafica extends JFrame {
    ConexionBD BD;
    ConexionBD BDSimple;
   ...
   // public XYSeries series =  new XYSeries("fds");
     ...
     */
    public IndicadorPromediosSimplesGrafica(final String title, String agente, String accion, int repeticion) {

        super(title+" "+agente+" for "+accion);

          
         final TimeSeries series1 = new TimeSeries("Price", Day.class);
          final TimeSeries series2 = new TimeSeries("Simple Moving Average", Day.class);
        ...
        BDSimple = new ConexionBD("mysql");
        ResultSet rSimple = BDSimple.consulta("SELECT a.fecha, a.valor FROM analisistecnico a, bursatil b where b.nombre = '"+agente+"' and b.IDAgente = a.IDAgente and a.IDProducto = "+idproducto+" and a.indicador = 'PMSimple' and a.repeticion = "+repeticion+" order by fecha");
        while(rSimple.next())
        {
                    fecha = rSimple.getInt(1);
                    actual = Calendar.getInstance();


                   precio = rSimple.getFloat(2);
                   //System.out.println(actual.get(Calendar.DATE) + " " +actual.get(Calendar.MONTH)+ " " + actual.get(Calendar.YEAR) + " : "+fecha);
                   actual.add(Calendar.DATE, fecha-1);
        ...
        final JFreeChart chart = ChartFactory.createTimeSeriesChart(
            "Simple Moving Average of "+accion,
            "Date",
            "Value",
            dataset,
            true,
  ...
  /*  public static void main(final String[] args) {
        ...
        final IndicadorPromediosSimplesGrafica demo = new IndicadorPromediosSimplesGrafica("Cantidad de Acciones del Agente", "aT1", "Ecopetrol");
        demo.pack();
