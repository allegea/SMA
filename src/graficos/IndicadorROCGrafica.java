 ...
 *
 */
public class IndicadorROCGrafica extends JFrame {
    ConexionBD BD;
    ConexionBD BDROC;
   ...
   // public XYSeries series =  new XYSeries("fds");
     ...
     */
    public IndicadorROCGrafica(final String title, String agente, String accion, int repeticion) {

        super(title+" "+agente+" for "+accion);

        
  
        final TimeSeries series1 = new TimeSeries("Middle Line", Day.class);
        final TimeSeries series2 = new TimeSeries("ROC", Day.class);
...
*/
        ...
        BDROC = new ConexionBD("mysql");
        ResultSet rROC = BDROC.consulta("SELECT a.fecha, a.valor FROM analisistecnico a, bursatil b where b.nombre = '"+agente+"' and b.IDAgente = a.IDAgente and a.IDProducto = "+idproducto+" and a.indicador = 'ROC' and a.repeticion = "+repeticion+" order by fecha");
        while(rROC.next())
        {
                    fecha = rROC.getInt(1);
                    actual = Calendar.getInstance();
                   ...
                   precio = rROC.getFloat(2);
                   //System.out.println(actual.get(Calendar.DATE) + " " +actual.get(Calendar.MONTH)+ " " + actual.get(Calendar.YEAR) + " : "+fecha);
                   actual.add(Calendar.DATE, fecha-1);

                   series1.add(new Day(actual.get(Calendar.DATE), actual.get(Calendar.MONTH)+1,  actual.get(Calendar.YEAR)), 100);
